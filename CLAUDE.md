...
### 一、時間標記
- 所有報告的時間戳記一律採用**台北時間（UTC+8）**
- 格式：`YYYY-MM-DD HH:MM（台北時間 UTC+8）`
- 禁止使用 UTC、GMT 或未指定時區的時間

### 二、報告內容規格
- 每份深度分析報告至少包含 **9 個章節**
- 每個章節正文至少 **300 字**
- **每一段落至少 300 字**（段落不得以短句帶過，必須充分論述、引用數據或分析脈絡）
- **第一章執行摘要**：僅使用繁體中文，禁止附加英文對照
- 每份報告末尾必須附上**完整參考文獻清單**

### 三、引用規範
- 引用格式統一採用 **APA 第7版**
- **每一筆引用必須附有網址（URL）或 DOI 碼**，不得僅列書名/作者
- 來源包含：學術期刊、政府報告、智庫報告、新聞調查報告
- 禁止捏造任何來源、URL 或 DOI

### 四、格式規範
- Markdown 表格為必要格式，分隔行（`|---|---|`）為語法要求，不得省略
- 章節標題使用 `##`（一級）、`###`（二級）層次清楚
- 禁止在報告內文使用無意義裝飾符號（如連續的 `----`、`====`、`****` 裝飾線）
- 表格僅用於結構化數據呈現，避免過度使用導致閱讀負擔

### 五、Obsidian 知識庫同步
- 每份深度分析完成後，**必須同步寫入 `wiki/events/`**（event 類型）或 `wiki/concepts/`（concept 類型）
- Frontmatter 欄位必須完整填寫（title、type、status、author、reviewer、date、source）
- 寫入後 git add + commit，確保 Obsidian 知識庫與 GitHub 同步

### 六、議題命名規則
- 單日議題資料夾格式：`YYYYMMDD_議題XX_議題名稱/`
- 各日綜合報告格式：`YYYYMMDD_中共軍事動態綜合分析報告.md`
- 跨議題綜合資料夾格式：`YYYYMMDD_各議題綜合分析報告/`（禁止使用議題編號）
